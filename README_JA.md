...
- **`fast`**（デフォルト） — 軽量なワンショット応答。高速・低コストで、単純な画像 / 動画のワンショット生成に適しています。
- **`thinking`** — 深い構造化推論で、先にプランニングしてから多段階の分析を行います。複雑なブランドシステムやマルチアセットのキャンペーン、**および音声 / 音楽 / BGM / TTS（読み上げ）生成すべて**に適しています（下記の注記を参照）。やや遅いですが品質が高い。

```bash
# 高速ワンショット（デフォルト）
python3 agent_skill.py chat --prompt "猫を描いて"

# 深い推論、または 音声 / 音楽 / BGM / TTS
...
> ⚠️ **音声 / 音楽 / BGM / TTS（読み上げ）には `--mode thinking` が必須です。** `fast` モードでは、Agent が最も近い非音声ツール（通常は動画モデル）を選んでしまい、音声ではなく動画ファイルが返ってきます。その間もクレジットは消費され、自動返金はありません。BGM・音楽・効果音・読み上げを生成したい場合は、必ず新しい thread を開いて `--mode thinking` を指定してください。
