...
```bash
python3 scripts/agent_skill.py project-add --project-id PROJECT_ID --name "My Project"
```

### 1. Send a message (reads project_id from local state)

```bash
python3 scripts/agent_skill.py chat --prompt "USER_PROMPT" --json --download
```

To override project: add `--project-id PROJECT_ID`
To continue a conversation: add `--thread-id THREAD_ID`
To list saved threads: `python3 scripts/agent_skill.py threads`

### 2. Create a project

```bash
python3 scripts/agent_skill.py create-project
```

### 3. Upload a file (local image/video → CDN URL)

```bash
python3 scripts/agent_skill.py upload --file /path/to/image.png
...
```bash
python3 scripts/agent_skill.py upload-artifact --project-id PROJECT_ID --url "ARTIFACT_URL" --type image
```
...
```bash
# Status
python3 scripts/agent_skill.py status --thread-id THREAD_ID

# Result (auto-syncs to gallery/canvas, idempotent)
python3 scripts/agent_skill.py result --thread-id THREAD_ID --json --download
```
...
python3 scripts/agent_skill.py chat --prompt "draw a cat" --json --download --output-dir /tmp/openclaw

# Download from existing result
python3 scripts/agent_skill.py result --thread-id THREAD_ID --download --output-dir /tmp/openclaw

# Download specific URLs
python3 scripts/agent_skill.py download --urls URL1 URL2 --output-dir /tmp/openclaw --prefix myimg
```
...
```bash
python3 scripts/agent_skill.py watch --prompt "generate 4 variations of a cyberpunk cat" --json
```
...
```bash
python3 scripts/agent_skill.py chat --prompt "generate ocean waves video using kling" --json --download
```
...
python3 scripts/agent_skill.py chat --prompt "draw a cat" --prefer-models '{"IMAGE":["generate_image_midjourney"]}' --json --download

# Prefer a specific video model
python3 scripts/agent_skill.py chat --prompt "generate ocean waves" --prefer-models '{"VIDEO":["generate_video_kling_3_0"]}' --json --download

# Combine image and video preferences
python3 scripts/agent_skill.py chat --prompt "create content" --prefer-models '{"IMAGE":["generate_image_seedream_3_0"],"VIDEO":["generate_video_kling_3_0"]}' --json --download
```
...
```bash
# Force upscale only
python3 scripts/agent_skill.py chat --prompt "upscale this image to 4K" --include-tools upscale_image --attachments "IMAGE_URL" --json --download

# Force a specific video model (no fallback to others)
python3 scripts/agent_skill.py chat --prompt "generate a video" --include-tools generate_video_kling_3_0 --json --download
```
...
python3 scripts/agent_skill.py chat --prompt "design a brand identity system for a sustainable coffee startup" --mode thinking --json --download

# Fast mode — quick one-shot
python3 scripts/agent_skill.py chat --prompt "draw a cat" --mode fast --json --download
```
